...
| `GET   /parent/children/{child_id}/analysis/concepts` | parent | Returns the single weakest trick category (most failures all-time). A failure = `previously_failed=true`. Returns `{ child_id, weakest_category, failed }`. `weakest_category` is `null` and `failed` is `0` if the child has no failures yet. |
