'use client'
// ─────────────────────────────────────────────────────────────
//  MathQuest · src/components/game/Zone3Game.tsx
//
//  React host for the Zone 3 "Iron Summit" Phaser scene.
//  Mirrors Zone2Game.tsx exactly — only theme, colors, and
//  event names differ.
// ─────────────────────────────────────────────────────────────
...
import { ZONE3_EVENTS } from '@/lib/phaser/Zone3Scene'
...
function TouchControls({ visible }: { visible: boolean }) {
  ...
  const getScene = () => (window as any).__zone3Scene
  ...
  return (
    <div className="absolute bottom-6 left-0 right-0 z-30 flex items-end justify-between px-6 pointer-events-none">
      <div className="flex gap-3 pointer-events-auto">
        <button type="button" className={`${btn} w-20 h-20 bg-stone-800/40 backdrop-blur-sm border-2 border-orange-500/40 text-white text-3xl shadow-lg shadow-orange-900/30`}
      ...
      <div className="pointer-events-auto">
        <button type="button" className={`${btn} w-24 h-24 bg-orange-500/80 backdrop-blur-sm border-2 border-orange-300 text-[#0d0a06] text-4xl shadow-xl shadow-orange-500/40`}
...
function ProgressHUD({ solved, total }: { solved: number; total: number }) {
  ...
  let barColor = ''
  if      (pct < 30) barColor = 'from-stone-500 to-stone-400'
  else if (pct < 70) barColor = 'from-orange-500 to-amber-400'
  else               barColor = 'from-emerald-400 to-teal-400'

  return (
    <div className="absolute top-4 left-4 z-20 select-none pointer-events-none">
      <div className="bg-[#1a0800]/70 backdrop-blur-sm rounded-2xl px-4 py-2.5 border border-orange-500/20">
        <div className="flex items-center gap-2 mb-1.5">
          <span className="text-orange-300 text-sm font-black tracking-wide">⚙️ Iron Summit</span>
          <span className="text-white/40 text-xs">Zone 3</span>
        </div>
        <div className="flex items-center gap-2 mb-1.5">
        ...
        <div className="flex gap-1">
          {Array.from({ length: total }).map((_, i) => (
            <div key={i} className={`w-2.5 h-2.5 rounded-full border transition-all duration-300
              ${i < solved ? 'bg-orange-400 border-orange-300' : 'bg-white/10 border-white/20'}`} />
          ))}
        </div>
      </div>
    </div>
...
function BossHUD({ phase, visible }: { phase: number; visible: boolean }) {
  ...
  return (
    <div className="absolute top-4 right-4 z-20 select-none pointer-events-none">
      <div className="bg-[#2d1000]/90 backdrop-blur-sm rounded-2xl px-4 py-2.5 border border-orange-500/50">
        <div className="text-orange-300 text-xs font-bold uppercase tracking-widest mb-1">⚡ Boss Battle</div>
        <div className="text-white text-sm font-black">Iron Titan</div>
        <div className="flex gap-1.5 mt-1.5">
          {[1, 2, 3].map(i => (
            <div key={i} className={`w-6 h-3 rounded-sm border transition-all duration-500
              ${i > phase ? 'bg-orange-500 border-orange-400' : 'bg-white/10 border-white/20'}`} />
          ))}
        </div>
        <div className="text-orange-400/70 text-[10px] mt-1 font-bold">Phase {phase} of 3</div>
      </div>
    </div>
  )
}
...
function CoinStreak({ coins, sessionCoins, streak, capReached, onTricks, tricksCount, onStory, hasStory }: {
  ...
  return (
    <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 flex gap-2 select-none">
      <div className={`pointer-events-none flex items-center gap-2 bg-[#1a0800]/60 backdrop-blur-sm rounded-full px-4 py-2 border ${capReached ? 'border-white/15' : 'border-orange-500/20'}`}>
        <div className={`w-4 h-4 rounded-full flex-shrink-0 ${capReached ? 'bg-white/25' : 'bg-orange-400 shadow-sm shadow-orange-400/50'}`} />
        <span className={`font-black text-sm tabular-nums ${capReached ? 'text-white/35' : 'text-orange-300'}`}>{coins}</span>
        {capReached && <span className="text-white/35 text-xs font-bold">🔒</span>}
      </div>
      {sessionCoins > 0 && (
        <div className="pointer-events-none flex items-center gap-1.5 bg-[#1a0800]/60 backdrop-blur-sm border border-amber-400/30 rounded-full px-3 py-2">
          <span className="text-amber-400 font-black text-xs tabular-nums">+{sessionCoins}</span>
          <span className="text-white/40 text-[10px] font-bold">session</span>
        </div>
      )}
      <div className="pointer-events-none flex items-center gap-2 bg-[#1a0800]/60 backdrop-blur-sm border border-orange-400/20 rounded-full px-4 py-2">
        <span>🔥</span>
      ...
      {onTricks && (
        <button
          type="button"
          onClick={onTricks}
          className="relative flex items-center gap-1.5 bg-[#1a0800]/60 hover:bg-orange-400/15 backdrop-blur-sm border border-orange-400/25 hover:border-orange-400/50 text-orange-300 rounded-full px-4 py-2 transition-all duration-150 active:scale-90"
          aria-label="View math tricks"
        >
          <span className="text-sm leading-none">✨</span>
          <span className="font-black text-sm">Tricks</span>
          {!!tricksCount && (
            <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-orange-400 text-[#0d0a06] text-[9px] font-black flex items-center justify-center">
              {tricksCount}
            </span>
          )}
        </button>
      ...
      {onStory && (
        <button
          type="button"
          onClick={onStory}
          className="relative flex items-center gap-1.5 bg-[#1a0800]/60 hover:bg-amber-400/10 backdrop-blur-sm border border-amber-400/25 hover:border-amber-400/50 text-amber-300 rounded-full px-4 py-2 transition-all duration-150 active:scale-90"
...
function KeyboardHint() {
  return (
    <div className="absolute bottom-4 right-4 z-10 select-none pointer-events-none">
      <div className="bg-[#1a0800]/40 backdrop-blur-sm rounded-xl px-3 py-1.5 border border-orange-500/15 text-white/30 text-[11px] flex gap-3">
        <span>← → or A D</span>
...
function MathModal({
  ...
  return (
    <div className="absolute inset-0 z-40 flex items-center justify-center bg-[#1a0800]/70 backdrop-blur-sm">
      <div className="w-full max-w-md mx-4 relative animate-[slideUp_0.3s_ease_forwards]">
        <button
          ...
          className="absolute -top-3 -right-3 z-50 w-10 h-10 rounded-full
                     bg-stone-800/40 hover:bg-stone-700/60 backdrop-blur-sm
...
function ZoneCompleteScreen({ onNext, onHub, sessionCoins }: { onNext: () => void; onHub: () => void; sessionCoins: number }) {
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-[#1a0800]/80 backdrop-blur-md">
      <div className="text-center px-8 animate-[zoomIn_0.5s_ease_forwards]">
        <div className="flex justify-center mb-2">
          <img
            src="/child.jpg"
            alt="You celebrating"
            className="w-28 h-28 rounded-full object-cover border-4 border-orange-400 shadow-xl shadow-orange-400/40 animate-[zoomIn_0.6s_0.1s_ease_both]"
          />
        </div>
        <div className="text-7xl mb-6 animate-bounce">🏆</div>
        <h1 className="text-5xl font-black text-white mb-3 tracking-tight">Zone 3 Complete!</h1>
        <p className="text-orange-300 text-xl font-bold mb-2">Iron Summit — Conquered!</p>
        <p className="text-white/50 text-base mb-10">You defeated the Iron Titan and solved all 8 obstacles!</p>
        <div className="flex justify-center gap-5 mb-10">
          {[
            { icon: '🪙', label: `+${sessionCoins} coins earned`, color: 'border-yellow-500/40 bg-yellow-950/60' },
            { icon: '⚙️', label: 'Forge Badge',                  color: 'border-orange-500/40 bg-orange-950/60' },
            { icon: '📖', label: 'Story Ch. 3',                  color: 'border-amber-500/40 bg-amber-950/60'   },
          ].map(r => (
        ...
        <div className="flex flex-col items-center gap-3">
          <button
            type="button"
            onClick={onNext}
            className="bg-orange-500 text-white font-black text-xl px-12 py-5 rounded-2xl
                       hover:bg-orange-400 active:scale-95 transition-all duration-150
                       shadow-xl shadow-orange-500/30"
          >Continue to Zone 4 →</button>
          <button
            type="button"
            onClick={onHub}
...
export default function Zone3Game() {
  ...
  // Seed coins and streak ONCE on initial load — same race-condition guard as Zone1/2Game
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      ...
      const { Zone3Scene } = await import('@/lib/phaser/Zone3Scene')
      ...
      fetchProblem()
        .then(({ problem, phase_signal }) => {
          if (problem) {
            ...
            dispatchToPhaser(ZONE3_EVENTS.ANSWER_RESULT, { correct: false, obstacleId: data.obstacleId })
          }
        })
        .catch(err => {
          console.error('[Zone3] fetchProblem error:', err)
          dispatchToPhaser(ZONE3_EVENTS.ANSWER_RESULT, { correct: false, obstacleId: data.obstacleId })
        })
    }

    const onBossPhase    = (e: Event) => { const d = (e as CustomEvent).detail; setBossPhase(d.phase); setBossVisible(true) }
    const onZoneComplete = () => { setZoneComplete(true); advanceZone(3).catch(() => {}) }

    window.addEventListener(ZONE3_EVENTS.SHOW_PROBLEM,  onShowProblem)
    window.addEventListener(ZONE3_EVENTS.BOSS_PHASE,    onBossPhase)
    window.addEventListener(ZONE3_EVENTS.ZONE_COMPLETE, onZoneComplete)
    return () => {
      window.removeEventListener(ZONE3_EVENTS.SHOW_PROBLEM,  onShowProblem)
      window.removeEventListener(ZONE3_EVENTS.BOSS_PHASE,    onBossPhase)
      window.removeEventListener(ZONE3_EVENTS.ZONE_COMPLETE, onZoneComplete)
    }
  }, [])
  ...
  const sendAnswer = useCallback((correct: boolean, obstacleId: string) => {
    ...
    dispatchToPhaser(ZONE3_EVENTS.ANSWER_RESULT, { correct, obstacleId })
  }, [])
  ...
  const handleCorrect = useCallback((result: AttemptResult) => {
    ...
    if (!wrongObstaclesRef.current.has(trigger.obstacleId)) {
      window.dispatchEvent(new CustomEvent(ZONE3_EVENTS.STREAK_INCREASE))
    }
  ...
  // ─────────────────────────────────────────────────────────
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-[#1a0800]">
      ...
      {activeTrigger && phaseSignal && (
        <TrickRevealModal trick={trickData} zone={3} onDismiss={handlePhaseSignalDismiss} />
      )}
      ...
      {zoneComplete && (
        <ZoneCompleteScreen
          onNext={() => { window.location.href = '/game/zone/4' }}
