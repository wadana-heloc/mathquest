...
function PasswordStrength({ password }: { password: string }) {
  ...
  // strength bar colors — same tokens, work on both light & dark
  const strengthColor = [
    "",
    "bg-coral",
    "bg-gold",
    ...
    <div className="mt-2">
      ...
      <div className="flex gap-1 mb-2">
        {[1, 2, 3, 4].map((i) => (
          <div
            ...
            className={[
              "h-1 flex-1 rounded-full transition-all duration-300",
              i <= score ? strengthColor : "",
            ].join(" ")}
            style={i > score ? { backgroundColor: "#E2E0DC" } : {}}
          />
        ))}
      </div>
      {/* check labels */}
      <div className="flex flex-wrap gap-x-3 gap-y-1">
        {checks.map((c) => (
          <span
            key={c.label}
            className="text-[11px] font-body transition-colors flex items-center gap-1"
            style={{ color: c.pass ? "#22C55E" : "#6B6A66", opacity: c.pass ? 1 : 0.6 }}
          >
            {c.pass ? "✓" : "○"} {c.label}
          </span>
        ))}
...
function ErrorBanner({ message }: { message: string }) {
  return (
    ...
    >
      <span className="text-base flex-shrink-0" style={{ color: "#F97316" }}>!</span>
      ...
      <AuthCard active="signup" animationClass="animate-slide-left">
        ...
        <div className="flex gap-2 flex-wrap mb-6">
          {FEATURES.map((f) => (
            <div
              key={f.text}
              className="flex items-center gap-1.5 text-xs font-body rounded-full px-3 py-1.5"
              style={{
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-4 md:space-y-5">
          ...
          <div>
            <label className="flex items-start gap-3 cursor-pointer group">
              <div
                onClick={() => setAgreed((v) => !v)}
                className={[
                  "mt-0.5 w-5 h-5 rounded flex-shrink-0 flex items-center justify-center",
                  "transition-all duration-150 cursor-pointer",
                ].join(" ")}
                style={{
                  border: agreed
                    ? "1.5px solid #E8B84B"
                    : "1.5px solid #E2E0DC",
                  backgroundColor: agreed ? "#E8B84B" : "#FFFFFF",
                }}
                role="checkbox"
                aria-checked={agreed}
                tabIndex={0}
          ...
          >
            <span className="text-base mt-0.5 flex-shrink-0" style={{ color: "#2DD4BF" }}>
              ℹ
            </span>
            <p
              className="text-xs sm:text-sm font-body leading-relaxed"
            ...
            >
              ...
              . You will add your child&apos;s profile from the dashboard.
            </p>
          </div>
